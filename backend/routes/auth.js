...
// Mock mode: password disimpan "mock:password123" → no bcrypt needed
// Real mode: bcrypt hash dari SQL Server
const router = require("express").Router();
const jwt = require("jsonwebtoken");
const { db } = require("../data/db");
const { verifyToken, PERMISSIONS } = require("./auth");

// Cek password: mock skip bcrypt, real pakai bcrypt
async function checkPassword(input, stored) {
  if (stored.startsWith("mock:")) {
    return input === stored.slice(5);
  }
  // Real mode: require bcryptjs hanya kalau dibutuhkan
