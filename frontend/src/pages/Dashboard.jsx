// pages/Dashboard.jsx — Supabase version, no backend
...
import { EmptyState, ErrorCard, SkeletonCard, SubmissionCard } from '../components/ui/index.jsx';
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', borderRadius:8, padding:'8px 12px' }}>
      ...
      {payload.map((p,i) => (
        <p key={i} style={{ color:p.color, fontSize:12, margin:'2px 0' }}>{p.name}: <strong>{p.value}</strong></p>
      ))}
    </div>
  );
};
...
export default function Dashboard() {
  ...
  const { filterOptions, metrics, submissions, loadingFilters, loadingMetrics, errorMetrics, refetchMetrics, refetchSubmissions, updateSubmission } = useDashboard({ tab:activeTab, plant, period, shift, line });

  const [showSubmissions, setShowSubmissions] = useState(false);

  // Set default filter saat options pertama kali load
  useEffect(() => {
  ...
  const setParam = (k, v) => setSearchParams(p => { p.set(k,v); return p; }, { replace:true });

  const handleTabChange = (tabId) => {
    setSearchParams(p => { p.set('tab',tabId); p.delete('shift'); p.delete('line'); return p; }, { replace:true });
  };

  const handlePlantChange = (pl) => {
    setSearchParams(p => { p.set('plant',pl); p.delete('line'); return p; }, { replace:true });
  };

  const handleApprove = async (id) => {
    const r = await updateSubmission(id, 'approved');
    if (!r.success) alert('Gagal: ' + r.message);
  };
  const handleReject = async (id) => {
    const r = await updateSubmission(id, 'rejected');
    if (!r.success) alert('Gagal: ' + r.message);
  };
    ...
    <div className="app-wrapper">
      <header className="header">
        ...
        <div className="header-right">
          ...
          <button className="btn-theme-toggle" onClick={toggleTheme}>{isDark ? '☀️' : '🌙'}</button>
          <div className="header-user">
            <div className="user-avatar" style={{ background:roleConfig?.bg, color:roleConfig?.color }}>{user?.avatar || user?.name?.[0]}</div>
            <div className="user-info">
              <span className="user-name">{user?.name}</span>
              <span className="user-role" style={{ color:roleConfig?.color }}>{roleConfig?.icon} {roleConfig?.label}</span>
            </div>
      ...
      <div className="app-body">
        ...
        <main className="main-content">
          <div className="filter-bar">
            ...
            <select className="filter-select" value={plant} onChange={e => handlePlantChange(e.target.value)} disabled={loadingFilters}>
              {filterOptions.plants.map(p => <option key={p} value={p} disabled={!canAccessPlant(p)}>{p}{!canAccessPlant(p)?' 🔒':''}</option>)}
            </select>
            <select className="filter-select" value={period} onChange={e => setParam('period',e.target.value)} disabled={loadingFilters}>
              {filterOptions.periods.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            <select className="filter-select" value={shift} onChange={e => setParam('shift',e.target.value)}>
              {filterOptions.shifts.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
            {filterOptions.lines?.length > 1 && (
              <select className="filter-select" value={line} onChange={e => setParam('line',e.target.value)}>
                {filterOptions.lines.map(l => <option key={l} value={l}>{l}</option>)}
              </select>
            )}
            <div className="filter-spacer"/>
            <button className={`btn-export ${exporting?'loading':''}`}
            ...
            <button className="btn-refresh" onClick={refetchMetrics} disabled={loadingMetrics}>{loadingMetrics ? '⏳' : '↺'}</button>
          ...
          <div className="scroll-area">
            {showSubmissions && (
              <section style={{ marginBottom:28 }}>
                <div className="section-title"><span className="section-dot" style={{background:'#FFD93D'}}/>Submissions — {activeTab} / {plant}</div>
            ...
            <section className="metrics-section">
              <div className="section-title">
                ...
                {(shift!=='All Shifts'||line!=='All Lines') && (
                  <span className="filter-active-indicator">
                    {shift!=='All Shifts'&&shift.split(' ')[1]}{line!=='All Lines'&&` · ${line}`}
                  </span>
                )}
              </div>
              ...
              {!loadingMetrics && !errorMetrics && metrics && (
                <div className="metrics-grid">
                  {metricList.map((m,i) => <MetricCard key={m.mkey} {...m} index={i} sparkData={metrics.chart}/>)}
                </div>
              )}
              {!loadingMetrics && !errorMetrics && !metrics && (
                <EmptyState icon="📭" title="Data tidak tersedia" desc={`${activeTab} / ${plant} / ${period}`}/>
              )}
            </section>
                ...
                <div className="charts-grid">
                  <div className="chart-card">
                    ...
                    <ResponsiveContainer width="100%" height={210}>
                      <BarChart data={metrics.chart} margin={{top:5,right:10,left:-10,bottom:5}}>
                        ...
                        {chartCfg.bars.map(b => <Bar key={b.k} dataKey={b.k} fill={b.c} name={b.n} radius={[4,4,0,0]}/>)}
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                  {chartCfg.line && (
                    <div className="chart-card">
                      ...
                      <ResponsiveContainer width="100%" height={210}>
                        <LineChart data={metrics.chart} margin={{top:5,right:10,left:-10,bottom:5}}>
                          ...
                          <Tooltip content={<CustomTooltip/>}/>
                          <Line type="monotone" dataKey={chartCfg.line.k} stroke={chartCfg.line.c} strokeWidth={2.5} dot={{fill:chartCfg.line.c,r:4}} activeDot={{r:6}} name={chartCfg.line.n}/>
                        </LineChart>
                      </ResponsiveContainer>
                    </div>
                  )}
                  <div className="chart-card">
                    ...
                    <div className="pie-wrapper">
                      <ResponsiveContainer width="100%" height={210}>
                        <PieChart>
                          <Pie data={PIE_DATA} cx="50%" cy="50%" innerRadius={55} outerRadius={82} paddingAngle={3} dataKey="value">
                            {PIE_DATA.map((_,i) => <Cell key={i} fill={PIE_COLORS[i]}/>)}
                          </Pie>
                          <Tooltip contentStyle={{background:'var(--bg-elevated)',border:'1px solid var(--border)',borderRadius:8,color:'var(--text-primary)'}} formatter={v=>[`${v}%`]}/>
                          <Legend wrapperStyle={{color:'var(--text-secondary)',fontSize:11}}/>
                        </PieChart>
                      </ResponsiveContainer>
                    </div>
